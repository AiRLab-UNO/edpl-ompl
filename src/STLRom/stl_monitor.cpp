...
namespace STLRom
{
    ...
    double STLMonitor::eval_smooth_rob(double tau, SmoothType type) {
        return eval_smooth_rob(start_time, end_time, tau, type);
    }

    double STLMonitor::eval_smooth_rob(double t_start, double t_end,
                                       double tau, SmoothType type) {
        if (formula) {
            formula->set_trace_data_ptr(data);
            formula->set_param_map_ptr(param_map);
            Signal::semantics = semantics;
            ...
            formula->reset();
            formula->set_horizon(t_start, t_end);
            rob = formula->compute_smooth_robustness(tau, type);
        }
        return rob;
    }
}