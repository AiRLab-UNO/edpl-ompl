...
namespace STLRom {
    ...
    public:
        ...
        // Smooth approximation of robustness using softmax or logsumexp.
        // tau is the temperature (higher -> closer to exact max/min).
        // Default implementation delegates to exact compute_robustness().
        virtual double compute_smooth_robustness(double tau, SmoothType type = SmoothType::LSE) {
    ...
    public:
        ...
        double compute_smooth_robustness(double tau, SmoothType type = SmoothType::LSE) override;
    ...
    public:
        ...
        double compute_robustness();
        // Atoms are arithmetic (no max/min), so smooth == exact
        double compute_smooth_robustness(double tau, SmoothType type = SmoothType::LSE) override {
            return compute_robustness();
        }
